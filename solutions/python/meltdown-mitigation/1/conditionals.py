...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    if efficiency >= 80.0:
        ...
        efficiency_state = 'orange'
    elif efficiency >= 30.0 and efficiency < 60.0:
        efficiency_state = 'red'
    else:
        efficiency_state = 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    normal_threshold_down = threshold - threshold * 0.1 
    normal_threshold_up = threshold + threshold * 0.1 
    total = temperature * neutrons_produced_per_second
    if total < low_threshold:
        status = 'LOW'
    elif total >= normal_threshold_down and total <= normal_threshold_up:
        status = 'NORMAL'
    else:
        status = 'DANGER'
    return status
